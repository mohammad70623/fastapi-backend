...
async def create_a_book(book_data:BookCreateModel, sessino:AsyncSession=Depends(get_session),user_details = Depends(access_token_bearer))->dict:
    new_book = await book_service.create_book(book_data, sessino)
    return new_book
